        ...
        , div
            ...
            , div [ class "col-auto" ]
                [ div [ class "mb-2" ]
                    ...
                    , input [ class "form-input", name "paxWeight", readonly True, value <| String.fromInt <| List.foldl (+) 0 <| List.map (paxWeight model) <| List.map (\pax -> pax.gender) <| model.manifest ] []
                    ]
                , div [ class "mb-2" ]
                    [ label [ class "form-label", for "bagWeight" ] [ text "Baggage weight" ]
                    , input [ class "form-input", name "bagWeight", readonly True, value <| String.fromInt <| (*) model.weightB <| List.foldl (+) 0 <| List.map (\pax -> pax.bags) <| model.manifest ] []
                    ]
                ]
            ]
        , div [ class "text-center" ]
            [ button [ class "btn", onClick GenerateManifest ] [ text "Generate Manifest" ] ]
        ]
